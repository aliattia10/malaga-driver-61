const translations = {
  en: {
    navbar: {
      locations: "Locations",
      services: "Services",
      book_now: "Book Now",
      testimonials: "Testimonials",
      about_us: "About Us",
      call_now: "Call Now"
    },
    locations: {
      custom_location: "Custom Location",
      málaga_airport: "Málaga Airport",
      málaga_city_center: "Málaga City Center"
    },
    hero: {
      subtitle: "Professional Private Drivers in Málaga",
      title_first: "Hire a Professional",
      title_second: "Private Driver in Málaga",
      description: "Safe and reliable transportation services with professional drivers, luxury vehicles, and personalized service throughout Málaga and Costa del Sol.",
      cta: "Book Your Private Driver"
    },
    services: {
      title: "Our Professional Driver Services",
      subtitle: "Reliable transportation solutions for any occasion",
      airport_transfers: {
        title: "Airport Transfers",
        description: "Punctual pickups and drop-offs at Málaga Airport and other destinations"
      },
      day_trips: {
        title: "Day Trips & Excursions",
        description: "Discover Andalusia with guided day trips to Granada, Seville, Córdoba, and more"
      },
      hourly: {
        title: "Hourly Driver Service",
        description: "Flexible transportation when you need it, by the hour or for the full day"
      },
      events: {
        title: "Events & Special Occasions",
        description: "Professional transportation for weddings, corporate events, and special days"
      }
    },
    pricing: {
      title: "Our Pricing",
      distance_based: "Our prices are based on distance (per kilometer):",
      example1: "48 km = 48 € cash",
      example2: "62 km = 62 € cash",
      and_so_on: "And so on...",
      base_price_valid: "This price is valid for 1 to 3 people.",
      additional_people: "Additional People",
      fourth_person: "4th person: +10 € extra",
      fifth_person: "5th person: +10 € additional",
      example_title: "Example:",
      example_3people: "60 km with 1 to 3 people → 60 €",
      example_4people: "60 km with 4 people → 60 € + 10 € = 70 €",
      example_5people: "60 km with 5 people → 60 € + 10 € + 10 € = 80 €",
      book_now: "Book now and travel without complications."
    },
    booking: {
      title: "Book Your Private Driver",
      subtitle: "Fill out the form below to request a quote or book a service",
      form: {
        name: "Your Name",
        email: "Email Address",
        phone: "Phone Number",
        pickup: "Pickup Location",
        dropoff: "Destination",
        date: "Pickup Date",
        time: "Pickup Time",
        passengers: "Number of Passengers",
        message: "Additional Information",
        submit: "Request Booking",
        success: "Booking request submitted! We'll contact you shortly.",
        error: "There was an error submitting your request. Please try again."
      }
    },
    about: {
      title: "About Us",
      subtitle: "Expert drivers with extensive local knowledge",
      description: "Experience the beautiful coastline and cultural richness of Málaga with professional drivers who truly know the region.",
      features: {
        experience: "Years of Experience",
        safety: "Safety First Approach",
        knowledge: "Local Knowledge & Expertise",
        service: "Personalized Service"
      }
    },
    footer: {
      contact: "Contact Us",
      address: "Address",
      phone: "Phone",
      email: "Email",
      copyright: "© 2023 Málaga Driver Hub. All rights reserved."
    },
    whatsapp: {
      message: "Contact us via WhatsApp",
      contact_now: "Contact us now"
    }
  },
  es: {
    navbar: {
      locations: "Ubicaciones",
      services: "Servicios",
      book_now: "Reservar Ahora",
      testimonials: "Testimonios",
      about_us: "Nosotros",
      call_now: "Llamar Ahora"
    },
    locations: {
      custom_location: "Ubicación Personalizada",
      málaga_airport: "Aeropuerto de Málaga",
      málaga_city_center: "Centro de Málaga"
    },
    hero: {
      subtitle: "Conductores Privados Profesionales en Málaga",
      title_first: "Contrate un Conductor",
      title_second: "Privado Profesional en Málaga",
      description: "Servicios de transporte seguros y confiables con conductores profesionales, vehículos de lujo y servicio personalizado en toda Málaga y la Costa del Sol.",
      cta: "Reserve Su Conductor Privado"
    },
    services: {
      title: "Nuestros Servicios de Conductor Profesional",
      subtitle: "Soluciones de transporte confiables para cualquier ocasión",
      airport_transfers: {
        title: "Traslados al Aeropuerto",
        description: "Recogidas y entregas puntuales en el Aeropuerto de Málaga y otros destinos"
      },
      day_trips: {
        title: "Excursiones de un Día",
        description: "Descubra Andalucía con excursiones guiadas a Granada, Sevilla, Córdoba y más"
      },
      hourly: {
        title: "Servicio por Horas",
        description: "Transporte flexible cuando lo necesite, por hora o para todo el día"
      },
      events: {
        title: "Eventos y Ocasiones Especiales",
        description: "Transporte profesional para bodas, eventos corporativos y días especiales"
      }
    },
    pricing: {
      title: "Nuestros Precios",
      distance_based: "Nuestros precios se basan en la distancia (por kilómetro):",
      example1: "48 km = 48 € en efectivo",
      example2: "62 km = 62 € en efectivo",
      and_so_on: "Y así sucesivamente…",
      base_price_valid: "Este precio es válido para 1 a 3 personas.",
      additional_people: "Personas Adicionales",
      fourth_person: "4ª persona: +10 € extra",
      fifth_person: "5ª persona: +10 € adicionales",
      example_title: "Ejemplo:",
      example_3people: "60 km con 1 a 3 personas → 60 €",
      example_4people: "60 km con 4 personas → 60 € + 10 € = 70 €",
      example_5people: "60 km con 5 personas → 60 € + 10 € + 10 € = 80 €",
      book_now: "Reserve ahora y viaje sin complicaciones."
    },
    booking: {
      title: "Reserve Su Conductor Privado",
      subtitle: "Complete el formulario a continuación para solicitar un presupuesto o reservar un servicio",
      form: {
        name: "Su Nombre",
        email: "Correo Electrónico",
        phone: "Número de Teléfono",
        pickup: "Lugar de Recogida",
        dropoff: "Destino",
        date: "Fecha de Recogida",
        time: "Hora de Recogida",
        passengers: "Número de Pasajeros",
        message: "Información Adicional",
        submit: "Solicitar Reserva",
        success: "¡Solicitud de reserva enviada! Nos pondremos en contacto con usted en breve.",
        error: "Hubo un error al enviar su solicitud. Por favor, inténtelo de nuevo."
      }
    },
    about: {
      title: "Sobre Nosotros",
      subtitle: "Conductores expertos con amplio conocimiento local",
      description: "Experimente la hermosa costa y la riqueza cultural de Málaga con conductores profesionales que realmente conocen la región.",
      features: {
        experience: "Años de Experiencia",
        safety: "Enfoque de Seguridad Primero",
        knowledge: "Conocimiento Local y Experiencia",
        service: "Servicio Personalizado"
      }
    },
    footer: {
      contact: "Contáctenos",
      address: "Dirección",
      phone: "Teléfono",
      email: "Correo Electrónico",
      copyright: "© 2023 Málaga Driver Hub. Todos los derechos reservados."
    },
    whatsapp: {
      message: "Contáctanos por WhatsApp",
      contact_now: "Contáctanos ahora"
    }
  }
};

export default translations;
